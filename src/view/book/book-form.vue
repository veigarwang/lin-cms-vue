          ...
          >
            <el-col :span="6">
              <el-form-item label="书籍图片" prop="cover">
                <upload-imgs ref="uploadEle1" :rules="rules" :multiple="false" :value="coverPreview" :max-num="1" accept="image/png,image/jpeg,image/gif,image/jpg"/>
              </el-form-item>
            </el-col>
            <el-col :span="6">
...
export default {
  ...
  async mounted() {
    ...
    if (this.edit_book_id) {
      this.form = await book.getBook(this.edit_book_id)
    ...
    async submitForm(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          if (this.edit_book_id) {
            try {
              res = await book.editBook(this.edit_book_id, this.form)
